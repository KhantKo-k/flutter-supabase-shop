...
class ProductListPage extends StatelessWidget {
  ...
  Widget _buildProductList(List<Product> products) {
    ...
    return SingleChildScrollView(
      child: Column(
        children: [_buildCountIndicator(length), _buildGridItems(products)],
      ),
    );
  }

  Widget _buildCountIndicator(int length) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12.0),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [Text('$length products', style: TextStyle(fontSize: 14))],
      ),
    );
  }

  Widget _buildGridItems(List<Product> products) {
